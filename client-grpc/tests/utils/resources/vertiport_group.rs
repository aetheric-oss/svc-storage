...
use svc_storage_client_grpc::resources::VertiportGroupLinkClient as LinkClient;

pub use svc_storage_client_grpc::link_service::Client;
pub use vertiport::*;

pub(crate) static NAME: &str = "vertiport_group_link";
...
pub async fn get_linked(client: &LinkClient, id: &String) -> group::List {
    let result = client.get_linked(Id { id: id.to_owned() }).await;
    it_debug!("{:?}", result);
    assert!(result.is_ok());
    let from_db: group::List = result.unwrap().into_inner();
...
pub async fn check_linked_ids(client: &LinkClient, id: &str, existing: &group::List) {
...
pub async fn replace_linked(client: &LinkClient, id: &str, link_ids: &mut Vec<String>) {
    ...
    let result = client
        .replace_linked(VertiportGroups {
            id: id.to_owned(),
            other_id_list: Some(IdList { ids }),
        })
        .await;
