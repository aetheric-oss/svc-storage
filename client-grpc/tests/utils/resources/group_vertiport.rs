...
use svc_storage_client_grpc::resources::GroupVertiportLinkClient as LinkClient;

pub use group::*;
pub use svc_storage_client_grpc::link_service::Client;

pub static NAME: &str = "group_vertiport_link";
...
pub async fn get_linked(client: &LinkClient, id: &String) -> vertiport::List {
    let result = client.get_linked(Id { id: id.to_owned() }).await;
    it_debug!("{:?}", result);
    assert!(result.is_ok());
    let from_db: vertiport::List = result.unwrap().into_inner();
...
pub async fn check_linked_ids(client: &LinkClient, id: &str, existing: &vertiport::List) {
...
pub async fn replace_linked(client: &LinkClient, id: &str, link_ids: &mut Vec<String>) {
    ...
    let result = client
        .replace_linked(GroupVertiports {
            id: id.to_owned(),
            other_id_list: Some(IdList { ids }),
        })
        .await;
...
/// Runs the full scenario of retrieving, inserting, deleting and searching
