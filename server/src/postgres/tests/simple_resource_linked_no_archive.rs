//! Simple Resource Linked `no archive` test helper functions

use crate::postgres::simple_resource::PsqlType as SimplePsqlType;

pub use crate::resources::test_util::simple_resource_linked_no_archive::*;
...
pub async fn test_delete_one(id: &str, linked_id: &str) {
    ...
    // check if we are a correct result from the is_archived function (should be false)
    ...
    // check if we are a correct result from the is_archived function (should still be `false` even
    // though the object is deleted. This resource does not have a `deleted_at` field)
    let result = obj.is_archived().await;
    assert!(
        !result,
        "Expected 'false' but got {} when checking is_archived on object after deletion.",
        result
    );
}
